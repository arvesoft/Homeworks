...
void main() {
  ...
  float specMax = max(0, dot( normalize(vertexNormal), normalize( ToLightVector + ToCameraVector ) ) );

  vec4 ambient = Ia * ka;
  vec4 diffuse = Id * kd * max( 0, dot( normalize(vertexNormal), normalize(ToLightVector) ) );
  vec4 specular = Is * ks * pow(specMax , specExp);

  // compute the color using the following equation
  color = vec4(clamp( textureColor.rgb * vec3( ambient + diffuse + specular ), 0.0, 1.0), 1.0);
}
